...
<body>
    ...
    $not_system_admin = false;
    $need_force_2fa = false;

    if ($user_profile) {
        if ($user_profile->m_system !== 'system_admin') {
            $not_system_admin = true;
        }
        // Legacy admin ก็ยังต้องบังคับใช้ 2FA
        if (empty($user_profile->m_system) && $user_profile->ref_pid == 1) {
            $not_system_admin = true;
        }
        ...
        if ($not_system_admin && !$is_2fa_active) {
            $need_force_2fa = true;
        }
    }
    ...
    function get_module_icon($module_code)
    {
        $icons = [
            ...
            'wastesystem' => 'fa-solid fa-recycle'
        ];
    ...
    function generate_system_url($module_id, $module_code, $is_trial)
    {
        ...
        $base_urls = [
            ...
            'wastesystem' => 'https://wastesystem.assystem.co.th/auth/login'
        ];
        ...
        <div id="force2faModal" class="force-2fa-modal">
            <div class="force-2fa-content">
                ...
                <p class="force-2fa-message">
                    ในฐานะ <strong>Super Administrator</strong> คุณจำเป็นต้องเปิดใช้งานการยืนยันตัวตน 2 ขั้นตอน (2FA)
                ...
                <button type="button" class="force-2fa-logout-btn"
                    onclick="window.location.href='<?php echo site_url('User/logout'); ?>'">
    ...
    <div id="errorModal" class="error-modal">
        <div class="error-modal-content">
            ...
            <p class="error-message">ท่านไม่มีสิทธิ์เข้าใช้งานระบบนี้ โปรดติดต่อผู้ดูแลระบบของ <span
                    id="tenant-name"><?php echo $this->session->userdata('tenant_name'); ?></span></p>
    ...
    <div class="container-custom">
        <header class="header">
            ...
            <p class="site-subtitle">ลดขั้นตอน ประหยัดเวลา และเพิ่มประสิทธิภาพการทำงานของเจ้าหน้าที่ ก้าวไปสู่ระบบราชการ
                4.0</p>
        </header>
        ...
        <div class="user-header">
            ...
            <div class="profile-actions">
                ...
                <?php elseif (!$is_2fa_active): ?>
                    <a href="<?php echo site_url('System_admin/user_profile'); ?>#security-section"
                        class="btn-profile btn-profile-security">
                        <i class="bi bi-shield-plus"></i>
                        ตั้งค่าความปลอดภัย
                        <span class="security-notification-dot"></span>
                    </a>
                <?php else: ?>
                    <a href="<?php echo site_url('System_admin/user_profile'); ?>#security-section"
                        class="btn-profile btn-profile-security-active">
                        <i class="bi bi-shield-check"></i>
                        จัดการความปลอดภัย
                    </a>
                <?php endif; ?>
                ...
                if ($module->status):
                    ...
                    <div class="card-wrapper <?php echo $wrapperClass; ?>">
                        ...
                        <a href="<?php echo $need_force_2fa ? 'javascript:show2FAReminder()' : generate_system_url($module->id, $module->code, $module->is_trial); ?>"
                            ...
                            data-tenant-id="<?php echo $this->session->userdata('tenant_id'); ?>" <?php if ($isBackOfficeModule && $backoffice_notifications_count > 0): ?>
                                title="ระบบ Back Office (มีการแจ้งเตือน <?php echo $backoffice_notifications_count; ?> รายการ)"
                            <?php elseif ($isWebboardModule && $webboard_notifications_count > 0): ?>
                                title="กระดานสนทนา (มีการแจ้งเตือน <?php echo $webboard_notifications_count; ?> รายการ)" <?php endif; ?>>
                ...
                foreach ($admin_modules as $module):
                    ...
                    <div class="card-wrapper <?php echo $wrapperClass; ?>">
                        ...
                        <a href="<?php echo $need_force_2fa ? 'javascript:show2FAReminder()' : $module['url']; ?>"
                            class="card <?php echo $module['color']; ?>" <?php if ($isReportsModule): ?>
                                title="ระบบ e-Service<?php echo $staff_unread_count > 0 ? ' (มีการแจ้งเตือน ' . $staff_unread_count . ' รายการ)' : ''; ?>"
                            <?php endif; ?>>
                            <div class="card-content">
                                <div class="icon-circle">
                                    <i
                                        class="<?php echo $module['icon']; ?> card-icon <?php echo $module['code'] === 'email_system' ? 'email-icon' : ''; ?>"></i>
                                </div>
                                <h2 class="card-title"><?php echo $module['name']; ?></h2>
                            </div>
                        </a>
        ...
        <div class="logout-container">
            <a href="<?php echo $need_force_2fa ? 'javascript:show2FAReminder()' : site_url('Home'); ?>"
                class="home-button">
        ...
        <div class="membership-terms-section">
            ...
            <div class="membership-terms-container">
                <a href=<?php echo base_url("policy/membership"); ?> target="_blank" class="membership-terms-link">
                    <i class="fas fa-file-contract"></i>
                    <span>ข้อกำหนดและเงื่อนไขการสมัครสมาชิก</span>
                </a>
            </div>
        </div>
        ...
        <div class="support">
            <div class="support-container">
                <span class="support-text">ติดต่อฝ่ายขาย</span>
    ...
    <script>
        ...
        const isSuperAdmin = <?php echo $not_system_admin ? 'true' : 'false'; ?>;
