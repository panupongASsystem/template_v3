...
<div class="row">
    <div class="col-6">
        <p class="path"><a class="path" href="<?php echo site_url('food_backend'); ?>">Home</a> / <a class="path" href="<?php echo site_url('food_backend'); ?>"><?= $food->food_name; ?></a> / <a class="path" href="#">ความคิดเห็น</a></p>
    </div>
    <div class="col-6">
    ...
    <tbody>
        ...
        <?php foreach ($rsCom as $rs) { ?>
            <tr role="row" class="comment-row">
                ...
                <td class="limited-text"><?= $rs->food_com_msg; ?></td>
                <td><?= $rs->food_com_by; ?></td>
                <td><?= date('d/m/Y H:i', strtotime($rs->food_com_datesave . '+543 years')) ?> น.</td>
                <td><a href="javascript:void(0);" onclick="confirmDelete(<?= $rs->food_com_id; ?>);" style="margin-left: 15%;"><i class="bi bi-trash fa-lg "></i></td>
                <script>
                    function confirmDelete(food_com_id) {
                        Swal.fire({
                            title: 'กดเพื่อยืนยัน?',
                            ...
                            if (result.isConfirmed) {
                                ...
                                fetch("<?= site_url('food_backend/del_com/'); ?>" + food_com_id)
            ...
            if (!empty($comReplyData)) {
                ...
                foreach ($comReplyData as $reply) { ?>
                    <tr class="reply-row" style="display: none;">
                        ...
                        <td class="reply"><?= $reply->food_com_reply_msg; ?></td>
                        <td><?= $reply->food_com_reply_by; ?></td>
                        <td><?= date('d/m/Y H:i', strtotime($reply->food_com_reply_datesave . '+543 years')) ?> น.</td>
                        <td><a href="javascript:void(0);" onclick="confirmDeleteReply(<?= $reply->food_com_reply_id; ?>);" style="margin-left: 15%;"><i class="bi bi-trash fa-lg "></i></td>
                        <script>
                            function confirmDeleteReply(replyId) {
                                Swal.fire({
                                    ...
                                    if (result.isConfirmed) {
                                        // ส่งคำขอ Ajax เพื่อลบข้อมูล
                                        fetch("<?= site_url('food_backend/del_com_reply/'); ?>" + replyId)
                                            .then(response => response.json())
                                            .then(data => {
                                                if (data.success) {
                                                    // หากการลบสำเร็จ รีเฟรชหน้า
