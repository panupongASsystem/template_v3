...
<script>
    // ส่งข้อมูล User System Level และ System Folders
    const currentUserSystem = '<?php echo $this->session->userdata('m_system'); ?>';
    const systemFolders = ['Admin', 'Departments', 'Shared', 'Users'];

    console.log('🔐 Current User System Level:', currentUserSystem);
    console.log('📁 System Folders:', systemFolders);
</script>
<div class="ml-72 p-8">
    <!-- Header -->
    <div class="flex justify-between items-center mb-8">
        <div>
            ...
            <p class="text-gray-600 mt-2">จัดการสิทธิ์ โฟลเดอร์และไฟล์ และจัดการไฟล์ใน Centralized Google Drive Storage
            </p>
        </div>
        <div class="flex space-x-3">

            <!-- <button onclick="showUploadModal()"
                class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors" id="uploadBtn"
            ...
            <button onclick="showCreateFolderModal()"
                ...
                id="createFolderBtn" <?php echo (!$system_storage || !$system_storage->folder_structure_created) ? 'disabled' : ''; ?>>
    ...
    <div class="bg-white rounded-lg shadow border border-gray-100" id="fileBrowserContainer">
        ...
        <div id="dropZoneOverlay"
            class="hidden absolute inset-0 z-50 bg-blue-500 bg-opacity-20 border-4 border-dashed border-blue-500 rounded-lg flex items-center justify-center backdrop-blur-sm">
    ...
    <?php if ($system_storage): ?>
        <div class="mt-8 bg-blue-50 border border-blue-200 rounded-lg p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-blue-700">
                <div>
                    <p><strong>Google Account:</strong>
                        <?php echo htmlspecialchars($system_storage->google_account_email ?? 'ไม่ได้ระบุ'); ?></p>
                    <p><strong>Storage Name:</strong>
                        <?php echo htmlspecialchars($system_storage->storage_name ?? 'Organization Storage'); ?></p>
                </div>
                <div>
                    <p><strong>สร้างเมื่อ:</strong>
                        <?php echo isset($system_storage->created_at) ? date('d/m/Y H:i', strtotime($system_storage->created_at)) : 'ไม่ทราบ'; ?>
                    </p>
        ...
        <div class="mb-4">
            ...
            <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-gray-400 transition-colors"
                ...
                <div onclick="document.getElementById('fileInput').click()" class="cursor-pointer">
                    ...
                    <p class="text-sm text-gray-500 mt-1">รองรับไฟล์: PDF, Word, Excel, PowerPoint, รูปภาพ, Text, ZIP
                    </p>
            ...
            <div class="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-5 gap-3">
                    ...
                    <div>
                        ...
                        <input type="date" id="newPermissionExpiry"
...
<div id="subfolderPermissionsModal"
    class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
...
<div id="subfolderAdvancedModal"
    class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg shadow-xl w-full max-w-5xl mx-4 max-h-screen overflow-y-auto">
        <!-- Header -->
        <div
            class="flex justify-between items-center p-6 border-b border-gray-200 bg-gradient-to-r from-purple-50 to-blue-50">
            <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <div class="flex justify-between items-center mb-3">
                        ...
                        <div class="flex items-center space-x-2">
                            <input type="checkbox" id="enableInheritanceAdvanced" checked
                                onchange="toggleAdvancedInheritance()">
...
<script>
    ...
    /**
     * ✅ ปรับปรุง: renderListView - ส่ง item.name ไปด้วย + เพิ่มการตรวจสอบสิทธิ์
     */
        ...
        <tr class="${rowClass} transition-colors" ${rowOnClick}>
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" onclick="event.stopPropagation();">
                ${isFolder ? `
                    <div class="flex items-center space-x-2">
                        ...
                        ${canManageSystemFolder(item.name) ? `
                            <button onclick="manageFolderPermissions('${item.id}', '${escapeHtml(item.name)}')" 
                        ...
                        ` : `
                            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-gray-300 text-gray-600 cursor-not-allowed" 
                                  title="เฉพาะ System Admin เท่านั้นที่สามารถจัดการสิทธิ์โฟลเดอร์ระบบได้">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" onclick="event.stopPropagation();">
                <div class="flex space-x-2">
                    ...
                    <button onclick="deleteItem('${item.id}', '${item.type}', '${escapeHtml(item.name)}')" class="text-red-600 hover:text-red-800" title="ลบ"><i class="fas fa-trash"></i></button>
                </div>
            </td>
        </tr>
    `;
    ...
    /**
     * ✅ ปรับปรุง: renderGridView - ส่ง item.name ไปด้วย + เพิ่มการตรวจสอบสิทธิ์
     */
            ...
            <div class="${cardClass}" ${cardOnClick}>
                ...
                <div class="mt-2 opacity-0 group-hover:opacity-100 transition-opacity" onclick="event.stopPropagation();">
                    <div class="flex justify-center space-x-1">
                        ${isFolder && canManageSystemFolder(item.name) ?
                    `<button onclick="manageFolderPermissions('${item.id}', '${escapeHtml(item.name)}')" class="p-1 text-blue-600 hover:bg-blue-50 rounded" title="จัดการสิทธิ์"><i class="fas fa-user-cog text-sm"></i></button>` : ''
                }
                        ${item.type === 'file' ?
                    `<button onclick="downloadFile('${item.id}', '${escapeHtml(item.name)}')" class="p-1 text-green-600 hover:bg-green-50 rounded" title="ดาวน์โหลด"><i class="fas fa-download text-sm"></i></button>` : ''
                        ...
                        <button onclick="deleteItem('${item.id}', '${item.type}', '${escapeHtml(item.name)}')" class="p-1 text-red-600 hover:bg-red-50 rounded" title="ลบ"><i class="fas fa-trash"></i></button>
                    </div>
                </div>
            </div>
        `;
    ...
    /**
     * ลบ Item (แสดง confirmation dialog)
     * ✅ แก้ไข: ส่ง item_name ที่ถูกต้องไปยัง backend
     */
    function deleteItem(itemId, itemType, itemName = '') {
        // ตรวจสอบสิทธิ์อีกครั้งก่อนลบ (double check)
        if (itemType === 'folder' && !canDeleteFolder(itemName)) {
            Swal.fire({
                title: 'ไม่มีสิทธิ์',
                html: `<div class="text-gray-700">
                <p>คุณไม่มีสิทธิ์ลบโฟลเดอร์นี้</p>
                <small class="text-gray-500 mt-2">ระดับสิทธิ์ปัจจุบัน: <strong>${currentUserSystem}</strong></small>
            </div>`,
                icon: 'error',
                confirmButtonText: 'ตกลง',
        ...
        if (itemType === 'file' && !canDeleteFile()) {
            Swal.fire({
                title: 'ไม่มีสิทธิ์',
                html: `<div class="text-gray-700">
                <p>คุณไม่มีสิทธิ์ลบไฟล์นี้</p>
                <small class="text-gray-500 mt-2">ระดับสิทธิ์ปัจจุบัน: <strong>${currentUserSystem}</strong></small>
            </div>`,
                icon: 'error',
                confirmButtonText: 'ตกลง',
        ...
        // แสดง confirmation
        const displayName = itemName || 'รายการนี้';
        const itemTypeText = itemType === 'folder' ? 'โฟลเดอร์' : 'ไฟล์';

        Swal.fire({
            title: 'ยืนยันการลบ',
            html: `
            <div class="text-left">
                <p>คุณต้องการลบ<strong class="text-red-600"> ${itemTypeText}</strong> นี้หรือไม่?</p>
                <div class="mt-3 p-3 bg-red-50 rounded border border-red-200">
                    <p class="font-semibold text-gray-800">${displayName}</p>
                </div>
                ${itemType === 'folder' ? '<p class="text-sm text-red-600 mt-3">⚠️ โฟลเดอร์และไฟล์ทั้งหมดภายในจะถูกลบ</p>' : ''}
                <p class="text-sm text-gray-500 mt-2">⚠️ การดำเนินการนี้ไม่สามารถย้อนกลับได้</p>
            </div>
        `,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonText: '<i class="fas fa-trash mr-2"></i>ยืนยันการลบ',
            cancelButtonText: 'ยกเลิก',
            ...
            if (result.isConfirmed) {
                performDeleteItem(itemId, itemType, itemName);
            }
        });
    }

    /**
     * ดำเนินการลบ Item จริง
     * ✅ แก้ไข: ปรับปรุงการส่งข้อมูลและการจัดการ error
     */
    function performDeleteItem(itemId, itemType, itemName) {
        const loadingToast = Swal.fire({
            title: 'กำลังลบ...',
            html: `<div class="text-center">
            <p>กรุณารอสักครู่</p>
            <small class="text-gray-500">กำลังดำเนินการลบ ${itemType === 'folder' ? 'โฟลเดอร์' : 'ไฟล์'}</small>
        </div>`,
        ...
        fetch(API_BASE_URL + 'delete_item', {
            ...
            body: `item_id=${encodeURIComponent(itemId)}&item_type=${encodeURIComponent(itemType)}&item_name=${encodeURIComponent(itemName || '')}`
        })
            .then(response => {
                console.log('Delete response status:', response.status, response.statusText);

                // ✅ จัดการ HTTP error codes ต่างๆ
                if (response.status === 403) {
                    return response.json().then(data => {
                        throw new Error(data.message || 'ไม่มีสิทธิ์ในการลบ');
                    });
                }

                if (response.status === 404) {
                    return response.json().then(data => {
                        throw new Error(data.message || 'ไม่พบรายการที่ต้องการลบ');
                    });
                }
                ...
                if (response.status === 500) {
                    return response.json().then(data => {
                        ...
                        throw new Error(data.message || 'เกิดข้อผิดพลาดภายในเซิร์ฟเวอร์');
                    }).catch(err => {
                        // ถ้า parse JSON ไม่ได้
                        throw new Error('เกิดข้อผิดพลาดภายในเซิร์ฟเวอร์ (HTTP 500)');
                    });
                }
            ...
            .then(data => {
                ...
                if (data.success) {
                    Swal.fire({
                        ...
                        position: 'top-end',
                        icon: 'success',
                        title: `ลบ${itemType === 'folder' ? 'โฟลเดอร์' : 'ไฟล์'}เรียบร้อยแล้ว`,
                        showConfirmButton: false,
                        timer: 3000,
                        timerProgressBar: true
                    });
            ...
            .catch(error => {
                ...
                Swal.fire({
                    ...
                    <p class="text-gray-700 mb-2">${error.message || 'ไม่สามารถเชื่อมต่อกับเซิร์ฟเวอร์ได้'}</p>
                    <small class="text-gray-500">กรุณาลองใหม่อีกครั้ง หรือติดต่อผู้ดูแลระบบ</small>
                </div>`,
                    icon: 'error',
                    confirmButtonText: 'ตกลง',
                    confirmButtonColor: '#3b82f6'
     ...
     * @returns {boolean}
     */
    function canDeleteFolder(folderName) {
        // ตรวจสอบว่าเป็น system folder หรือไม่
        const isSytemFolder = systemFolders.includes(folderName);

        if (isSytemFolder) {
            // System folders (Admin, Departments, Shared, Users): 
            // เฉพาะ system_admin เท่านั้น
            return currentUserSystem === 'system_admin';
        } else {
            // Folders อื่นๆ: 
            // super_admin และ system_admin ลบได้
            return currentUserSystem === 'super_admin' || currentUserSystem === 'system_admin';
        }
    }
    ...
    /**
     * ✅ ฟังก์ชันตรวจสอบว่า User สามารถจัดการสิทธิ์โฟลเดอร์ระบบได้หรือไม่
     * @param {string} folderName - ชื่อโฟลเดอร์
     * @returns {boolean}
     */
    function canManageSystemFolder(folderName) {
        // ตรวจสอบว่าเป็น System Folder หรือไม่
        const isSystemFolder = systemFolders.includes(folderName);

        // ถ้าไม่ใช่ System Folder = ทุกคนจัดการได้
        if (!isSystemFolder) {
            return true;
        }

        // ถ้าเป็น System Folder = ต้องเป็น system_admin เท่านั้น
        return currentUserSystem === 'system_admin';
    }

    /**
     * Render ปุ่มลบตามสิทธิ์
     * @param {object} item - ข้อมูล item (file/folder)
     * @returns {string} HTML ของปุ่มลบ
     */
    function renderDeleteButton(item) {
        let canDelete = false;

        if (item.type === 'folder') {
            canDelete = canDeleteFolder(item.name);
        } else if (item.type === 'file') {
            canDelete = canDeleteFile();
        }

        // ถ้าไม่มีสิทธิ์ ไม่แสดงปุ่ม
        if (!canDelete) {
            return '';
        }

        // ถ้ามีสิทธิ์ แสดงปุ่มลบ
        return `
        <button onclick="deleteItem('${item.id}', '${item.type}', '${escapeHtml(item.name)}')" 
    ...
    /**
     * Render ปุ่มลบสำหรับ Grid View
     * @param {object} item - ข้อมูล item (file/folder)
     * @returns {string} HTML ของปุ่มลบ
     */
    function renderDeleteButtonGrid(item) {
        let canDelete = false;
    ...
    /**
     * ✅ แก้ไขแล้ว - อัพเดตจำนวนไฟล์อย่างถูกต้อง
     * - ไม่เขียนทับ System Total ที่ PHP render มา
     * - อัพเดตเฉพาะเมื่อมีการเปลี่ยนแปลงจริง (upload/delete)
     */
    function updateFileCount() {
        const fileCount = fileListData.filter(item => item.type === 'file').length;

        // ✅ อัพเดต "Current Folder" counter แทน
        const currentFolderFilesElement = document.getElementById('currentFolderFiles');
        if (currentFolderFilesElement) {
            currentFolderFilesElement.textContent = fileCount;
        }

        // ⚠️ ไม่แตะ System Total Counter เลย
        // const totalFilesElement = document.getElementById('totalFilesCount');
        // totalFilesElement.textContent = fileCount; // ❌ ลบบรรทัดนี้ออก
    }
...
<script>
    ...
    `;
        ...
        // ✅ ตรวจสอบสิทธิ์ก่อนแสดงปุ่มจัดการ
        if (canManageSystemFolder(item.name)) {
            // ✅ User มีสิทธิ์ = แสดงปุ่มจัดการปกติ
        ...
        `;
            ...
            // ❌ User ไม่มีสิทธิ์ = แสดงสถานะล็อค
            html += `
            <div class="mt-1">
                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-gray-300 text-gray-600 cursor-not-allowed" 
                      title="เฉพาะ System Admin เท่านั้นที่สามารถจัดการสิทธิ์โฟลเดอร์ระบบได้">
