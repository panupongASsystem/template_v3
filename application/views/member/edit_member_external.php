<div class="ml-72 p-8">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h2 class="text-2xl font-semibold text-gray-800">แก้ไขสมาชิกภายนอก</h2>
            <p class="text-gray-600">แก้ไขข้อมูลของ: <?php echo $member->mp_fname; ?> <?php echo $member->mp_lname; ?></p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-md">
        <!-- Form -->
        <?php echo form_open_multipart('System_member/edit_member_external_db', ['id' => 'edit-member-external-form']); ?>
        <input type="hidden" name="mp_id" value="<?php echo $member->mp_id; ?>">
                ...
                <div class="p-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="email" id="mp_email" name="mp_email" required value="<?php echo $member->mp_email; ?>"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label for="mp_password" class="block text-lg font-medium text-gray-700 mb-1">รหัสผ่านใหม่</label>
                                <input type="password" id="mp_password" name="mp_password" minlength="6"
                                    class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                <p class="text-sm text-gray-500 mt-1">เว้นว่างไว้หากไม่ต้องการเปลี่ยนรหัสผ่าน</p>
                            </div>
                            <div>
                                <label for="mp_password_confirm" class="block text-lg font-medium text-gray-700 mb-1">ยืนยันรหัสผ่านใหม่</label>
                                <input type="password" id="mp_password_confirm" name="mp_password_confirm" minlength="6"
                                    class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            </div>
                        </div>
                    </div>
                ...
                <div class="p-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" id="mp_fname" name="mp_fname" required value="<?php echo $member->mp_fname; ?>"
                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        </div>
                        <div>
                            <label for="mp_lname" class="block text-lg font-medium text-gray-700 mb-1">นามสกุล <span class="text-red-600">*</span></label>
                            <input type="text" id="mp_lname" name="mp_lname" required value="<?php echo $member->mp_lname; ?>"
                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        </div>
                        <div>
                            ...
                            <input type="text" id="mp_number" name="mp_number" required maxlength="13" pattern="[0-9]{13}" value="<?php echo $member->mp_number; ?>"
                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        </div>
                        <div>
                            <label for="mp_phone" class="block text-lg font-medium text-gray-700 mb-1">เบอร์โทรศัพท์ <span class="text-red-600">*</span></label>
                            <input type="text" id="mp_phone" name="mp_phone" required maxlength="10" pattern="[0-9]{10}" value="<?php echo $member->mp_phone; ?>"
                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        </div>
                        <div class="md:col-span-2">
                            ...
                            <textarea id="mp_address" name="mp_address" rows="3"
                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"><?php echo $member->mp_address; ?></textarea>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="flex items-center justify-center">
                            <div class="w-32 h-32 bg-gray-100 rounded-full flex items-center justify-center border">
                                <img id="preview-image" src="<?php echo !empty($member->mp_img) ? base_url('docs/img/' . $member->mp_img) : base_url('docs/default_user.png'); ?>" alt="Preview" class="h-full w-full object-cover rounded-full">
                            </div>
                        </div>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(event) {
        ...
        // ตรวจสอบรหัสผ่านตรงกัน (ถ้ามีการกรอก)
    ...
    const originalEmail = '<?php echo $member->mp_email; ?>';
    emailInput.addEventListener('blur', function() {
        const email = this.value;
        if (email && email !== originalEmail && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
            fetch('<?php echo site_url("System_member/check_email_exists_external"); ?>', {
                ...
                body: 'email=' + encodeURIComponent(email) + '&mp_id=<?php echo $member->mp_id; ?>'
            })
    ...
    const originalIdCard = '<?php echo $member->mp_number; ?>';
    idCardInput.addEventListener('blur', function() {
        const idCard = this.value;
        if (idCard && idCard !== originalIdCard && idCard.length === 13 && /^\d+$/.test(idCard)) {
            fetch('<?php echo site_url("System_member/check_id_card_exists_external"); ?>', {
                ...
                body: 'id_card=' + encodeURIComponent(idCard) + '&mp_id=<?php echo $member->mp_id; ?>'
            })
            .then(response => response.json())
            .then(data => {
                if (data.exists) {
