...
class Pbsv_e_book_model extends CI_Model
{
    ...
    public function add()
    {
        ...
        $this->log_model->add_log(
            'เพิ่ม',
            'ดาวโหลดแบบฟอร์ม E-BOOK',
            $pbsv_e_book_data['pbsv_e_book_name'],
            $pbsv_e_book_id,
            array(
...
public function del_pdf($pdf_id)
{
    ...
    $this->db->select('pbsv_e_book_pdf_pdf, pbsv_e_book_pdf_ref_id');
    $this->db->where('pbsv_e_book_pdf_id', $pdf_id);
    $query = $this->db->get('tbl_pbsv_e_book_pdf');
    $file_data = $query->row();

    // ดึงข้อมูลเอกสารหลักสำหรับ log (สมมติว่ามี function read หรือ get_by_id)
    $pbsv_e_book_data = null;
    if ($file_data && $file_data->pbsv_e_book_pdf_ref_id) {
        $pbsv_e_book_data = $this->db->get_where('tbl_pbsv_e_book', array('pbsv_e_book_id' => $file_data->pbsv_e_book_pdf_ref_id))->row();
    }
    ...
    $this->log_model->add_log(
        'ลบ',
        'ดาวโหลดแบบฟอร์ม E-BOOK', // ปรับชื่อเมนูตามที่ใช้จริง
        'ไฟล์ PDF: ' . $file_data->pbsv_e_book_pdf_pdf . ' จากเอกสาร: ' . ($pbsv_e_book_data ? $pbsv_e_book_data->pbsv_e_book_name : 'ไม่ระบุ'),
        $file_data->pbsv_e_book_pdf_ref_id,
        array(
            'file_type' => 'PDF', 
            'file_name' => $file_data->pbsv_e_book_pdf_pdf,
            'pdf_id' => $pdf_id
        )
    );
    // ==================================================================
...
{
    // ดึงข้อมูลไฟล์และ ref_id สำหรับ logging
    $this->db->select('pbsv_e_book_file_doc, pbsv_e_book_file_ref_id');
    ...
    $pbsv_e_book_data = null;
    if ($file_data && $file_data->pbsv_e_book_file_ref_id) {
        $pbsv_e_book_data = $this->db->get_where('tbl_pbsv_e_book', array('pbsv_e_book_id' => $file_data->pbsv_e_book_file_ref_id))->row();
    }
    ...
    $this->log_model->add_log(
        'ลบ',
        'ดาวโหลดแบบฟอร์ม E-BOOK', // ปรับชื่อเมนูตามที่ใช้จริง
        'ไฟล์ DOC: ' . $file_data->pbsv_e_book_file_doc . ' จากเอกสาร: ' . ($pbsv_e_book_data ? $pbsv_e_book_data->pbsv_e_book_name : 'ไม่ระบุ'),
        $file_data->pbsv_e_book_file_ref_id,
        array(
            'file_type' => 'DOC', 
            'file_name' => $file_data->pbsv_e_book_file_doc,
            'doc_id' => $doc_id
        )
    );
    // ==================================================================
...
{
    // ดึงข้อมูลไฟล์และ ref_id สำหรับ logging
    $this->db->select('pbsv_e_book_img_img, pbsv_e_book_img_ref_id');
    ...
    // ดึงข้อมูลเอกสารหลักสำหรับ log
    $pbsv_e_book_data = null;
    if ($file_data && $file_data->pbsv_e_book_img_ref_id) {
        $pbsv_e_book_data = $this->db->get_where('tbl_pbsv_e_book', array('pbsv_e_book_id' => $file_data->pbsv_e_book_img_ref_id))->row();
    }
    ...
    $this->log_model->add_log(
        'ลบ',
        'ดาวโหลดแบบฟอร์ม E-BOOK', // ปรับชื่อเมนูตามที่ใช้จริง
        'รูปภาพ: ' . $file_data->pbsv_e_book_img_img . ' จากเอกสาร: ' . ($pbsv_e_book_data ? $pbsv_e_book_data->pbsv_e_book_name : 'ไม่ระบุ'),
        $file_data->pbsv_e_book_img_ref_id,
        array(
            'file_type' => 'IMAGE', 
            'file_name' => $file_data->pbsv_e_book_img_img,
            'img_id' => $file_id
        )
    );
    // ==================================================================
    ...
    public function edit($pbsv_e_book_id)
    {
        ...
        $old_data = $this->read($pbsv_e_book_id);
        ...
        $changes = array();
        if ($old_data) {
            if ($old_data->pbsv_e_book_name != $data['pbsv_e_book_name']) {
                $changes['pbsv_e_book_name'] = array(
                    'old' => $old_data->pbsv_e_book_name,
                    'new' => $data['pbsv_e_book_name']
                );
            }
            if ($old_data->pbsv_e_book_detail != $data['pbsv_e_book_detail']) {
                $changes['pbsv_e_book_detail'] = array(
                    'old' => 'มีการเปลี่ยนแปลง',
                    'new' => 'รายละเอียดใหม่'
                );
            }
            if ($old_data->pbsv_e_book_date != $data['pbsv_e_book_date']) {
                $changes['pbsv_e_book_date'] = array(
                    'old' => $old_data->pbsv_e_book_date,
                    'new' => $data['pbsv_e_book_date']
                );
            }
            if ($old_data->pbsv_e_book_link != $data['pbsv_e_book_link']) {
                $changes['pbsv_e_book_link'] = array(
                    'old' => $old_data->pbsv_e_book_link,
                    'new' => $data['pbsv_e_book_link']
                );
            }
        }
        ...
        if (!empty($imgs_data)) {
            ...
            $files_info['image_names'] = array_column($imgs_data, 'pbsv_e_book_img_img');
        }
        if (!empty($pdf_data)) {
            $files_info['pdfs_added'] = count($pdf_data);
            $files_info['pdf_names'] = array_column($pdf_data, 'pbsv_e_book_pdf_pdf');
        }
        if (!empty($doc_data)) {
            $files_info['docs_added'] = count($doc_data);
            $files_info['doc_names'] = array_column($doc_data, 'pbsv_e_book_file_doc');
        }
    ...
    public function del_pbsv_e_book($pbsv_e_book_id)
{
    ...
    $pbsv_e_book_data = $this->db->get_where('tbl_pbsv_e_book', array('pbsv_e_book_id' => $pbsv_e_book_id))->row();

    // ลบรูปภาพหลัก
    if ($pbsv_e_book_data && $pbsv_e_book_data->pbsv_e_book_img) {
        $old_file_path = './docs/img/' . $pbsv_e_book_data->pbsv_e_book_img;
        if (file_exists($old_file_path)) {
    ...
    if ($pbsv_e_book_data) {
        $this->log_model->add_log(
            'ลบ',
            'ดาวโหลดแบบฟอร์ม E-BOOK', // ปรับชื่อเมนูตามที่ใช้จริง
            $pbsv_e_book_data->pbsv_e_book_name ?? 'เอกสารรายได้แผนดิน',
            $pbsv_e_book_id,
            array(
                'deleted_date' => date('Y-m-d H:i:s'),
                'main_image' => $pbsv_e_book_data->pbsv_e_book_img ?? null,
                'document_type' => 'pbsv_e_book_main'
            )
        );
    }
    // ====================================================================
