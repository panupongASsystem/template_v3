...
public function del_pdf($pdf_id)
{
    ...
    $this->db->select('pbsv_oppr_pdf_pdf, pbsv_oppr_pdf_ref_id');
    ...
    $pbsv_oppr_data = null;
    if ($file_data && $file_data->pbsv_oppr_pdf_ref_id) {
        $pbsv_oppr_data = $this->db->get_where('tbl_pbsv_oppr', array('pbsv_oppr_id' => $file_data->pbsv_oppr_pdf_ref_id))->row();
    }
    ...
    $this->log_model->add_log(
        'ลบ',
        'งานอาสาสมัครป้องกันภัยฝ่ายพลเรือน (อปพร.)', // ปรับชื่อเมนูตามที่ใช้จริง
        'ไฟล์ PDF: ' . $file_data->pbsv_oppr_pdf_pdf . ' จากเอกสาร: ' . ($pbsv_oppr_data ? $pbsv_oppr_data->pbsv_oppr_name : 'ไม่ระบุ'),
        $file_data->pbsv_oppr_pdf_ref_id,
        array(
            'file_type' => 'PDF', 
            'file_name' => $file_data->pbsv_oppr_pdf_pdf,
            'pdf_id' => $pdf_id
        )
    );
    // ==================================================================
...
{
    // ดึงข้อมูลไฟล์และ ref_id สำหรับ logging
    $this->db->select('pbsv_oppr_file_doc, pbsv_oppr_file_ref_id');
    ...
    $pbsv_oppr_data = null;
    if ($file_data && $file_data->pbsv_oppr_file_ref_id) {
        $pbsv_oppr_data = $this->db->get_where('tbl_pbsv_oppr', array('pbsv_oppr_id' => $file_data->pbsv_oppr_file_ref_id))->row();
    }
    ...
    $this->log_model->add_log(
        'ลบ',
        'งานอาสาสมัครป้องกันภัยฝ่ายพลเรือน (อปพร.)', // ปรับชื่อเมนูตามที่ใช้จริง
        'ไฟล์ DOC: ' . $file_data->pbsv_oppr_file_doc . ' จากเอกสาร: ' . ($pbsv_oppr_data ? $pbsv_oppr_data->pbsv_oppr_name : 'ไม่ระบุ'),
        $file_data->pbsv_oppr_file_ref_id,
        array(
            'file_type' => 'DOC', 
            'file_name' => $file_data->pbsv_oppr_file_doc,
            'doc_id' => $doc_id
        )
    );
    // ==================================================================
...
{
    // ดึงข้อมูลไฟล์และ ref_id สำหรับ logging
    $this->db->select('pbsv_oppr_img_img, pbsv_oppr_img_ref_id');
    ...
    // ดึงข้อมูลเอกสารหลักสำหรับ log
    $pbsv_oppr_data = null;
    if ($file_data && $file_data->pbsv_oppr_img_ref_id) {
        $pbsv_oppr_data = $this->db->get_where('tbl_pbsv_oppr', array('pbsv_oppr_id' => $file_data->pbsv_oppr_img_ref_id))->row();
    }
    ...
    $this->log_model->add_log(
        'ลบ',
        'งานอาสาสมัครป้องกันภัยฝ่ายพลเรือน (อปพร.)', // ปรับชื่อเมนูตามที่ใช้จริง
        'รูปภาพ: ' . $file_data->pbsv_oppr_img_img . ' จากเอกสาร: ' . ($pbsv_oppr_data ? $pbsv_oppr_data->pbsv_oppr_name : 'ไม่ระบุ'),
        $file_data->pbsv_oppr_img_ref_id,
        array(
            'file_type' => 'IMAGE', 
            'file_name' => $file_data->pbsv_oppr_img_img,
            'img_id' => $file_id
        )
    );
    // ==================================================================
    ...
    public function edit($pbsv_oppr_id)
    {
        ...
        $old_data = $this->read($pbsv_oppr_id);
        ...
        $changes = array();
        if ($old_data) {
            if ($old_data->pbsv_oppr_name != $data['pbsv_oppr_name']) {
                $changes['pbsv_oppr_name'] = array(
                    'old' => $old_data->pbsv_oppr_name,
                    'new' => $data['pbsv_oppr_name']
                );
            }
            if ($old_data->pbsv_oppr_detail != $data['pbsv_oppr_detail']) {
                $changes['pbsv_oppr_detail'] = array(
                    'old' => 'มีการเปลี่ยนแปลง',
                    'new' => 'รายละเอียดใหม่'
                );
            }
            if ($old_data->pbsv_oppr_date != $data['pbsv_oppr_date']) {
                $changes['pbsv_oppr_date'] = array(
                    'old' => $old_data->pbsv_oppr_date,
                    'new' => $data['pbsv_oppr_date']
                );
            }
            if ($old_data->pbsv_oppr_link != $data['pbsv_oppr_link']) {
                $changes['pbsv_oppr_link'] = array(
                    'old' => $old_data->pbsv_oppr_link,
                    'new' => $data['pbsv_oppr_link']
                );
            }
        }
        ...
        if (!empty($imgs_data)) {
            ...
            $files_info['image_names'] = array_column($imgs_data, 'pbsv_oppr_img_img');
        }
        if (!empty($pdf_data)) {
            $files_info['pdfs_added'] = count($pdf_data);
            $files_info['pdf_names'] = array_column($pdf_data, 'pbsv_oppr_pdf_pdf');
        }
        if (!empty($doc_data)) {
            $files_info['docs_added'] = count($doc_data);
            $files_info['doc_names'] = array_column($doc_data, 'pbsv_oppr_file_doc');
        }
    ...
    public function del_pbsv_oppr($pbsv_oppr_id)
{
    ...
    $pbsv_oppr_data = $this->db->get_where('tbl_pbsv_oppr', array('pbsv_oppr_id' => $pbsv_oppr_id))->row();

    // ลบรูปภาพหลัก
    if ($pbsv_oppr_data && $pbsv_oppr_data->pbsv_oppr_img) {
        $old_file_path = './docs/img/' . $pbsv_oppr_data->pbsv_oppr_img;
        if (file_exists($old_file_path)) {
    ...
    if ($pbsv_oppr_data) {
        $this->log_model->add_log(
            'ลบ',
            'งานอาสาสมัครป้องกันภัยฝ่ายพลเรือน (อปพร.)', // ปรับชื่อเมนูตามที่ใช้จริง
            $pbsv_oppr_data->pbsv_oppr_name ?? 'เอกสารรายได้แผนดิน',
            $pbsv_oppr_id,
            array(
                'deleted_date' => date('Y-m-d H:i:s'),
                'main_image' => $pbsv_oppr_data->pbsv_oppr_img ?? null,
                'document_type' => 'pbsv_oppr_main'
            )
        );
    }
    // ====================================================================
