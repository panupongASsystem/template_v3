...
class Announce_model extends CI_Model
{
    ...
    public function read_pdf($announce_id)
    {
        ...
        usort($results, function ($a, $b) {
            return strnatcmp($a->announce_pdf_pdf, $b->announce_pdf_pdf);
        });
    ...
    public function read_doc($announce_id)
    {
        ...
        usort($results, function ($a, $b) {
            return strnatcmp($a->announce_file_doc, $b->announce_file_doc);
        });
    ...
    public function read_img($announce_id)
    {
        ...
        $results = $query->result();
        usort($results, function ($a, $b) {
            return strnatcmp($a->announce_img_img, $b->announce_img_img);
        });
        return $results;
    }
    ...
    public function del_pdf($pdf_id)
    {
        ...
        $this->db->select('announce_pdf_pdf, announce_pdf_ref_id');
        $this->db->where('announce_pdf_id', $pdf_id);
        $query = $this->db->get('tbl_announce_pdf');
        $file_data = $query->row();

        // บันทึก log การลบไฟล์ PDF =============================================
        $old_data = $this->read($file_data->announce_pdf_ref_id);
        // =======================================================================
        ...
        $this->log_model->add_log(
            'ลบ',
            'ประกาศ',
            'ไฟล์ PDF: ' . $file_data->announce_pdf_pdf . ' จาก: ' . ($old_data ? $old_data->announce_name : 'ไม่ระบุ'),
            $file_data->announce_pdf_ref_id,
            array('file_type' => 'PDF', 'file_name' => $file_data->announce_pdf_pdf)
        );
        // =======================================================================
    ...
    public function del_doc($doc_id)
    {
        ...
        $this->db->select('announce_file_doc, announce_file_ref_id');
        $this->db->where('announce_file_id', $doc_id);
        $query = $this->db->get('tbl_announce_file');
        $file_data = $query->row();

        // บันทึก log การลบไฟล์ DOC ========================================
        $old_data = $this->read($file_data->announce_file_ref_id);
        // ================================================================
        ...
        $this->log_model->add_log(
            'ลบ',
            'ประกาศ',
            'ไฟล์ DOC: ' . $file_data->announce_file_doc . ' จาก: ' . ($old_data ? $old_data->announce_name : 'ไม่ระบุ'),
            $file_data->announce_file_ref_id,
            array('file_type' => 'DOC', 'file_name' => $file_data->announce_file_doc)
        );
        // ================================================================
        $this->session->set_flashdata('del_success', TRUE);
    }
    ...
    public function del_img($file_id)
    {
        ...
        $this->db->select('announce_img_img, announce_img_ref_id');
        $this->db->where('announce_img_id', $file_id);
        $query = $this->db->get('tbl_announce_img');
        $file_data = $query->row();

        // บันทึก log การลบรูปภาพ =================================================
        $old_data = $this->read($file_data->announce_img_ref_id);
        // =======================================================================
        ...
        $this->log_model->add_log(
            'ลบ',
            'ประกาศ',
            'รูปภาพ: ' . $file_data->announce_img_img . ' จาก: ' . ($old_data ? $old_data->announce_name : 'ไม่ระบุ'),
            $file_data->announce_img_ref_id,
            array('file_type' => 'IMAGE', 'file_name' => $file_data->announce_img_img)
        );
        // =======================================================================
        $this->session->set_flashdata('del_success', TRUE);
    }
    ...
    public function edit($announce_id)
    {
        ...
        $old_data = $this->read($announce_id);
        ...
        $changes = array();
        if ($old_data) {
            if ($old_data->announce_name != $data['announce_name']) {
                $changes['announce_name'] = array(
                    'old' => $old_data->announce_name,
                    'new' => $data['announce_name']
                );
            }
            if ($old_data->announce_detail != $data['announce_detail']) {
                $changes['announce_detail'] = array(
                    'old' => 'มีการเปลี่ยนแปลง',
                    'new' => 'รายละเอียดใหม่'
                );
            }
            if ($old_data->announce_date != $data['announce_date']) {
                $changes['announce_date'] = array(
                    'old' => $old_data->announce_date,
                    'new' => $data['announce_date']
                );
            }
            if ($old_data->announce_link != $data['announce_link']) {
                $changes['announce_link'] = array(
                    'old' => $old_data->announce_link,
                    'new' => $data['announce_link']
                );
            }
        }
        ...
        if (!empty($imgs_data)) {
            ...
            $files_info['image_names'] = array_column($imgs_data, 'announce_img_img');
        }
        if (!empty($pdf_data)) {
            $files_info['pdfs_added'] = count($pdf_data);
            $files_info['pdf_names'] = array_column($pdf_data, 'announce_pdf_pdf');
        }
        if (!empty($doc_data)) {
            $files_info['docs_added'] = count($doc_data);
            $files_info['doc_names'] = array_column($doc_data, 'announce_file_doc');
        }
