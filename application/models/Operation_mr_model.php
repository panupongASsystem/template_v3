...
class Operation_mr_model extends CI_Model
{
    ...
    public function add()
    {
        ...
        $this->log_model->add_log(
            'เพิ่ม',
            'การจัดการความเสี่ยง',
            $operation_mr_data['operation_mr_name'],
            $operation_mr_id,
            array(
    ...
    public function read_pdf($operation_mr_id)
    {
        ...
        usort($results, function ($a, $b) {
            return strnatcmp($a->operation_mr_pdf_pdf, $b->operation_mr_pdf_pdf);
        });
    ...
    public function read_doc($operation_mr_id)
    {
        ...
        usort($results, function ($a, $b) {
            return strnatcmp($a->operation_mr_file_doc, $b->operation_mr_file_doc);
        });
    ...
    public function read_img($operation_mr_id)
    {
        ...
        $results = $query->result();
        usort($results, function ($a, $b) {
            return strnatcmp($a->operation_mr_img_img, $b->operation_mr_img_img);
        });
        return $results;
    }
    ...
    public function del_pdf($pdf_id)
    {
        ...
        $this->db->select('operation_mr_pdf_pdf, operation_mr_pdf_ref_id');
        $this->db->where('operation_mr_pdf_id', $pdf_id);
        $query = $this->db->get('tbl_operation_mr_pdf');
        $file_data = $query->row();
		
		// บันทึก log การลบไฟล์ PDF =============================================
        $old_data = $this->read($file_data->operation_mr_pdf_ref_id);
        // =======================================================================
        ...
        $this->log_model->add_log(
            'ลบ',
            'การจัดการความเสี่ยง',
            'ไฟล์ PDF: ' . $file_data->operation_mr_pdf_pdf . ' จาก: ' . ($old_data ? $old_data->operation_mr_name : 'ไม่ระบุ'),
            $file_data->operation_mr_pdf_ref_id,
            array('file_type' => 'PDF', 'file_name' => $file_data->operation_mr_pdf_pdf)
        );
        // =======================================================================
    ...
    public function del_doc($doc_id)
    {
        ...
        $this->db->select('operation_mr_file_doc, operation_mr_file_ref_id');
        $this->db->where('operation_mr_file_id', $doc_id);
        $query = $this->db->get('tbl_operation_mr_file');
        $file_data = $query->row();
		
		// บันทึก log การลบไฟล์ DOC ========================================
        $old_data = $this->read($file_data->operation_mr_file_ref_id);
        // ================================================================
        ...
        $this->log_model->add_log(
            'ลบ',
            'การจัดการความเสี่ยง',
            'ไฟล์ DOC: ' . $file_data->operation_mr_file_doc . ' จาก: ' . ($old_data ? $old_data->operation_mr_name : 'ไม่ระบุ'),
            $file_data->operation_mr_file_ref_id,
            array('file_type' => 'DOC', 'file_name' => $file_data->operation_mr_file_doc)
        );
        // ================================================================
        $this->session->set_flashdata('del_success', TRUE);
    }
    ...
    public function del_img($file_id)
    {
        ...
        $this->db->select('operation_mr_img_img, operation_mr_img_ref_id');
        $this->db->where('operation_mr_img_id', $file_id);
        $query = $this->db->get('tbl_operation_mr_img');
        $file_data = $query->row();
		
		// บันทึก log การลบรูปภาพ =================================================
        $old_data = $this->read($file_data->operation_mr_img_ref_id);
        // =======================================================================
        ...
        $this->log_model->add_log(
            'ลบ',
            'การจัดการความเสี่ยง',
            'รูปภาพ: ' . $file_data->operation_mr_img_img . ' จาก: ' . ($old_data ? $old_data->operation_mr_name : 'ไม่ระบุ'),
            $file_data->operation_mr_img_ref_id,
            array('file_type' => 'IMAGE', 'file_name' => $file_data->operation_mr_img_img)
        );
        // =======================================================================
        $this->session->set_flashdata('del_success', TRUE);
    }
    ...
    public function edit($operation_mr_id)
    {
        ...
        $old_data = $this->read($operation_mr_id);
        ...
        $changes = array();
        if ($old_data) {
            if ($old_data->operation_mr_name != $data['operation_mr_name']) {
                $changes['operation_mr_name'] = array(
                    'old' => $old_data->operation_mr_name,
                    'new' => $data['operation_mr_name']
                );
            }
            if ($old_data->operation_mr_detail != $data['operation_mr_detail']) {
                $changes['operation_mr_detail'] = array(
                    'old' => 'มีการเปลี่ยนแปลง',
                    'new' => 'รายละเอียดใหม่'
                );
            }
            if ($old_data->operation_mr_date != $data['operation_mr_date']) {
                $changes['operation_mr_date'] = array(
                    'old' => $old_data->operation_mr_date,
                    'new' => $data['operation_mr_date']
                );
            }
            if ($old_data->operation_mr_link != $data['operation_mr_link']) {
                $changes['operation_mr_link'] = array(
                    'old' => $old_data->operation_mr_link,
                    'new' => $data['operation_mr_link']
                );
            }
        }
        ...
        if (!empty($imgs_data)) {
            ...
            $files_info['image_names'] = array_column($imgs_data, 'operation_mr_img_img');
        }
        if (!empty($pdf_data)) {
            $files_info['pdfs_added'] = count($pdf_data);
            $files_info['pdf_names'] = array_column($pdf_data, 'operation_mr_pdf_pdf');
        }
        if (!empty($doc_data)) {
            $files_info['docs_added'] = count($doc_data);
            $files_info['doc_names'] = array_column($doc_data, 'operation_mr_file_doc');
        }
    ...
    public function del_operation_mr($operation_mr_id)
    {
        ...
        if ($old_document) {
            $this->log_model->add_log(
                'ลบ',
                'การจัดการความเสี่ยง',
                $old_document->operation_mr_name,
                $operation_mr_id,
