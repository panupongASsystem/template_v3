...
class Operation_am_hr_model extends CI_Model
{
    ...
    public function add()
    {
        ...
        $this->log_model->add_log(
            ...
            $operation_am_hr_data['operation_am_hr_name'],
            $operation_am_hr_id,
            array(
    ...
    public function read_pdf($operation_am_hr_id)
    {
        ...
        usort($results, function ($a, $b) {
            return strnatcmp($a->operation_am_hr_pdf_pdf, $b->operation_am_hr_pdf_pdf);
        });
    ...
    public function read_doc($operation_am_hr_id)
    {
        ...
        usort($results, function ($a, $b) {
            return strnatcmp($a->operation_am_hr_file_doc, $b->operation_am_hr_file_doc);
        });
    ...
    public function read_img($operation_am_hr_id)
    {
        ...
        $results = $query->result();
        usort($results, function ($a, $b) {
            return strnatcmp($a->operation_am_hr_img_img, $b->operation_am_hr_img_img);
        });
        return $results;
    }
    ...
    public function del_pdf($pdf_id)
    {
        ...
        $this->db->select('operation_am_hr_pdf_pdf, operation_am_hr_pdf_ref_id');
        $this->db->where('operation_am_hr_pdf_id', $pdf_id);
        $query = $this->db->get('tbl_operation_am_hr_pdf');
        $file_data = $query->row();
		
		// บันทึก log การลบไฟล์ PDF =============================================
        $old_data = $this->read($file_data->operation_am_hr_pdf_ref_id);
        // =======================================================================
        ...
        $this->log_model->add_log(
            'ลบ',
            'การดำเนินการบริหารทรัพยากรบุคคล',
            'ไฟล์ PDF: ' . $file_data->operation_am_hr_pdf_pdf . ' จาก: ' . ($old_data ? $old_data->operation_am_hr_name : 'ไม่ระบุ'),
            $file_data->operation_am_hr_pdf_ref_id,
            array('file_type' => 'PDF', 'file_name' => $file_data->operation_am_hr_pdf_pdf)
        );
        // =======================================================================
    ...
    public function del_doc($doc_id)
    {
        ...
        $this->db->select('operation_am_hr_file_doc, operation_am_hr_file_ref_id');
        $this->db->where('operation_am_hr_file_id', $doc_id);
        $query = $this->db->get('tbl_operation_am_hr_file');
        $file_data = $query->row();
		
		// บันทึก log การลบไฟล์ DOC ========================================
        $old_data = $this->read($file_data->operation_am_hr_file_ref_id);
        // ================================================================
        ...
        $this->log_model->add_log(
            'ลบ',
            'การดำเนินการบริหารทรัพยากรบุคคล',
            'ไฟล์ DOC: ' . $file_data->operation_am_hr_file_doc . ' จาก: ' . ($old_data ? $old_data->operation_am_hr_name : 'ไม่ระบุ'),
            $file_data->operation_am_hr_file_ref_id,
            array('file_type' => 'DOC', 'file_name' => $file_data->operation_am_hr_file_doc)
        );
        // ================================================================
        $this->session->set_flashdata('del_success', TRUE);
    }
    ...
    public function del_img($file_id)
    {
        ...
        $this->db->select('operation_am_hr_img_img, operation_am_hr_img_ref_id');
        $this->db->where('operation_am_hr_img_id', $file_id);
        $query = $this->db->get('tbl_operation_am_hr_img');
        $file_data = $query->row();
		
		// บันทึก log การลบรูปภาพ =================================================
        $old_data = $this->read($file_data->operation_am_hr_img_ref_id);
        // =======================================================================
        ...
        $this->log_model->add_log(
            'ลบ',
            'การดำเนินการบริหารทรัพยากรบุคคล',
            'รูปภาพ: ' . $file_data->operation_am_hr_img_img . ' จาก: ' . ($old_data ? $old_data->operation_am_hr_name : 'ไม่ระบุ'),
            $file_data->operation_am_hr_img_ref_id,
            array('file_type' => 'IMAGE', 'file_name' => $file_data->operation_am_hr_img_img)
        );
        // =======================================================================
        $this->session->set_flashdata('del_success', TRUE);
    }
    ...
    public function edit($operation_am_hr_id)
    {
        ...
        $old_data = $this->read($operation_am_hr_id);
        ...
        $changes = array();
        if ($old_data) {
            if ($old_data->operation_am_hr_name != $data['operation_am_hr_name']) {
                $changes['operation_am_hr_name'] = array(
                    'old' => $old_data->operation_am_hr_name,
                    'new' => $data['operation_am_hr_name']
                );
            }
            if ($old_data->operation_am_hr_detail != $data['operation_am_hr_detail']) {
                $changes['operation_am_hr_detail'] = array(
                    'old' => 'มีการเปลี่ยนแปลง',
                    'new' => 'รายละเอียดใหม่'
                );
            }
            if ($old_data->operation_am_hr_date != $data['operation_am_hr_date']) {
                $changes['operation_am_hr_date'] = array(
                    'old' => $old_data->operation_am_hr_date,
                    'new' => $data['operation_am_hr_date']
                );
            }
            if ($old_data->operation_am_hr_link != $data['operation_am_hr_link']) {
                $changes['operation_am_hr_link'] = array(
                    'old' => $old_data->operation_am_hr_link,
                    'new' => $data['operation_am_hr_link']
                );
            }
        }
        ...
        if (!empty($imgs_data)) {
            ...
            $files_info['image_names'] = array_column($imgs_data, 'operation_am_hr_img_img');
        }
        if (!empty($pdf_data)) {
            $files_info['pdfs_added'] = count($pdf_data);
            $files_info['pdf_names'] = array_column($pdf_data, 'operation_am_hr_pdf_pdf');
        }
        if (!empty($doc_data)) {
            $files_info['docs_added'] = count($doc_data);
            $files_info['doc_names'] = array_column($doc_data, 'operation_am_hr_file_doc');
        }
    ...
    public function del_operation_am_hr($operation_am_hr_id)
    {
        ...
        if ($old_document) {
            $this->log_model->add_log(
                'ลบ',
                'การดำเนินการบริหารทรัพยากรบุคคล',
                $old_document->operation_am_hr_name,
                $operation_am_hr_id,
