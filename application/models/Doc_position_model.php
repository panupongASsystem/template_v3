...
class Doc_position_model extends CI_Model
{
    ...
    public function read_docs($table_name, $item_id, $id_field, $ref_field, $doc_name_field, $order_field)
    {
        try {
            ...
            if ($query->num_rows() > 0) {
                log_message('debug', "Found {$query->num_rows()} documents in {$table_name} for {$ref_field}={$item_id}");
                return $query->result();
            } else {
                log_message('debug', "No documents found in {$table_name} for {$ref_field}={$item_id}");
                return [];
            }
        } catch (Exception $e) {
            log_message('error', "Error reading documents: " . $e->getMessage());
            return [];
        }
    }

    public function update_doc_order($table_name, $id_field, $order_field, $doc_order)
    {
        try {
            // ตรวจสอบข้อมูลที่รับเข้ามา
            if (empty($table_name) || empty($id_field) || empty($order_field) || empty($doc_order) || !is_array($doc_order)) {
                log_message('error', "Invalid data for update_doc_order");
            ...
            log_message('error', "Exception in update_doc_order: " . $e->getMessage());
