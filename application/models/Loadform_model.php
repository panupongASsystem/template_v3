...
class Loadform_model extends CI_Model
{
    ...
    public function read_pdf($loadform_id)
    {
        ...
        usort($results, function ($a, $b) {
            return strnatcmp($a->loadform_pdf_pdf, $b->loadform_pdf_pdf);
        });
    ...
    public function read_doc($loadform_id)
    {
        ...
        usort($results, function ($a, $b) {
            return strnatcmp($a->loadform_file_doc, $b->loadform_file_doc);
        });
    ...
    public function read_img($loadform_id)
    {
        ...
        $results = $query->result();
        usort($results, function ($a, $b) {
            return strnatcmp($a->loadform_img_img, $b->loadform_img_img);
        });
        return $results;
    }
    ...
    public function del_pdf($pdf_id)
    {
        ...
        $this->db->select('loadform_pdf_pdf, loadform_pdf_ref_id');
        $this->db->where('loadform_pdf_id', $pdf_id);
        $query = $this->db->get('tbl_loadform_pdf');
        $file_data = $query->row();
		
		// บันทึก log การลบไฟล์ PDF =============================================
        $old_data = $this->read($file_data->loadform_pdf_ref_id);
        // =======================================================================
        ...
        $this->log_model->add_log(
            'ลบ',
            'ดาวน์โหลดแบบฟอร์ม',
            'ไฟล์ PDF: ' . $file_data->loadform_pdf_pdf . ' จาก: ' . ($old_data ? $old_data->loadform_name : 'ไม่ระบุ'),
            $file_data->loadform_pdf_ref_id,
            array('file_type' => 'PDF', 'file_name' => $file_data->loadform_pdf_pdf)
        );
        // =======================================================================
    ...
    public function del_doc($doc_id)
    {
        ...
        $this->db->select('loadform_file_doc, loadform_file_ref_id');
        $this->db->where('loadform_file_id', $doc_id);
        $query = $this->db->get('tbl_loadform_file');
        $file_data = $query->row();
		
		// บันทึก log การลบไฟล์ DOC ========================================
        $old_data = $this->read($file_data->loadform_file_ref_id);
        // ================================================================
        ...
        $this->log_model->add_log(
            'ลบ',
            'ดาวน์โหลดแบบฟอร์ม',
            'ไฟล์ DOC: ' . $file_data->loadform_file_doc . ' จาก: ' . ($old_data ? $old_data->loadform_name : 'ไม่ระบุ'),
            $file_data->loadform_file_ref_id,
            array('file_type' => 'DOC', 'file_name' => $file_data->loadform_file_doc)
        );
        // ================================================================
        $this->session->set_flashdata('del_success', TRUE);
    }
    ...
    public function del_img($file_id)
    {
        ...
        $this->db->select('loadform_img_img, loadform_img_ref_id');
        $this->db->where('loadform_img_id', $file_id);
        $query = $this->db->get('tbl_loadform_img');
        $file_data = $query->row();
		
		 // บันทึก log การลบรูปภาพ =================================================
        $old_data = $this->read($file_data->loadform_img_ref_id);
        // =======================================================================
        ...
        $this->log_model->add_log(
            'ลบ',
            'ดาวน์โหลดแบบฟอร์ม',
            'รูปภาพ: ' . $file_data->loadform_img_img . ' จาก: ' . ($old_data ? $old_data->loadform_name : 'ไม่ระบุ'),
            $file_data->loadform_img_ref_id,
            array('file_type' => 'IMAGE', 'file_name' => $file_data->loadform_img_img)
        );
        // =======================================================================
        $this->session->set_flashdata('del_success', TRUE);
    }
    ...
    public function edit($loadform_id)
    {
        ...
        $old_data = $this->read($loadform_id);
        ...
        $changes = array();
        if ($old_data) {
            if ($old_data->loadform_name != $data['loadform_name']) {
                $changes['loadform_name'] = array(
                    'old' => $old_data->loadform_name,
                    'new' => $data['loadform_name']
                );
            }
            if ($old_data->loadform_detail != $data['loadform_detail']) {
                $changes['loadform_detail'] = array(
                    'old' => 'มีการเปลี่ยนแปลง',
                    'new' => 'รายละเอียดใหม่'
                );
            }
            if ($old_data->loadform_date != $data['loadform_date']) {
                $changes['loadform_date'] = array(
                    'old' => $old_data->loadform_date,
                    'new' => $data['loadform_date']
                );
            }
            if ($old_data->loadform_link != $data['loadform_link']) {
                $changes['loadform_link'] = array(
                    'old' => $old_data->loadform_link,
                    'new' => $data['loadform_link']
                );
            }
        }
        ...
        if (!empty($imgs_data)) {
            ...
            $files_info['image_names'] = array_column($imgs_data, 'loadform_img_img');
        }
        if (!empty($pdf_data)) {
            $files_info['pdfs_added'] = count($pdf_data);
            $files_info['pdf_names'] = array_column($pdf_data, 'loadform_pdf_pdf');
        }
        if (!empty($doc_data)) {
            $files_info['docs_added'] = count($doc_data);
            $files_info['doc_names'] = array_column($doc_data, 'loadform_file_doc');
        }
