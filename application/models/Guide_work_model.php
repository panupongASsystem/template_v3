...
class Guide_work_model extends CI_Model
{
    ...
    public function add()
    {
        ...
        $this->log_model->add_log(
            'เพิ่ม',
            'คู่มือการปฏิบัติงาน',
            $guide_work_data['guide_work_name'],
            $guide_work_id,
            array(
    ...
    public function read_pdf($guide_work_id)
    {
        ...
        usort($results, function ($a, $b) {
            return strnatcmp($a->guide_work_pdf_pdf, $b->guide_work_pdf_pdf);
        });
    ...
    public function read_doc($guide_work_id)
    {
        ...
        usort($results, function ($a, $b) {
            return strnatcmp($a->guide_work_file_doc, $b->guide_work_file_doc);
        });
    ...
    public function read_img($guide_work_id)
    {
        ...
        $results = $query->result();
        usort($results, function ($a, $b) {
            return strnatcmp($a->guide_work_img_img, $b->guide_work_img_img);
        });
        return $results;
    }
    ...
    public function del_pdf($pdf_id)
    {
        ...
        $this->db->select('guide_work_pdf_pdf, guide_work_pdf_ref_id');
        $this->db->where('guide_work_pdf_id', $pdf_id);
        $query = $this->db->get('tbl_guide_work_pdf');
        $file_data = $query->row();
		
		// บันทึก log การลบไฟล์ PDF =============================================
        $old_data = $this->read($file_data->guide_work_pdf_ref_id);
        // =======================================================================
        ...
        $this->log_model->add_log(
            'ลบ',
            'คู่มือการปฏิบัติงาน',
            'ไฟล์ PDF: ' . $file_data->guide_work_pdf_pdf . ' จาก: ' . ($old_data ? $old_data->guide_work_name : 'ไม่ระบุ'),
            $file_data->guide_work_pdf_ref_id,
            array('file_type' => 'PDF', 'file_name' => $file_data->guide_work_pdf_pdf)
        );
        // =======================================================================
    ...
    public function del_doc($doc_id)
    {
        ...
        $this->db->select('guide_work_file_doc, guide_work_file_ref_id');
        $this->db->where('guide_work_file_id', $doc_id);
        $query = $this->db->get('tbl_guide_work_file');
        $file_data = $query->row();
		
		// บันทึก log การลบไฟล์ DOC ========================================
        $old_data = $this->read($file_data->guide_work_file_ref_id);
        // ================================================================
        ...
        $this->log_model->add_log(
            'ลบ',
            'คู่มือการปฏิบัติงาน',
            'ไฟล์ DOC: ' . $file_data->guide_work_file_doc . ' จาก: ' . ($old_data ? $old_data->guide_work_name : 'ไม่ระบุ'),
            $file_data->guide_work_file_ref_id,
            array('file_type' => 'DOC', 'file_name' => $file_data->guide_work_file_doc)
        );
        // ================================================================
        $this->session->set_flashdata('del_success', TRUE);
    }
    ...
    public function del_img($file_id)
    {
        ...
        $this->db->select('guide_work_img_img, guide_work_img_ref_id');
        $this->db->where('guide_work_img_id', $file_id);
        $query = $this->db->get('tbl_guide_work_img');
        $file_data = $query->row();
		
		// บันทึก log การลบรูปภาพ =================================================
        $old_data = $this->read($file_data->guide_work_img_ref_id);
        // =======================================================================
        ...
        $this->log_model->add_log(
            'ลบ',
            'คู่มือการปฏิบัติงาน',
            'รูปภาพ: ' . $file_data->guide_work_img_img . ' จาก: ' . ($old_data ? $old_data->guide_work_name : 'ไม่ระบุ'),
            $file_data->guide_work_img_ref_id,
            array('file_type' => 'IMAGE', 'file_name' => $file_data->guide_work_img_img)
        );
        // =======================================================================
        $this->session->set_flashdata('del_success', TRUE);
    }
    ...
    public function edit($guide_work_id)
    {
        ...
        $old_data = $this->read($guide_work_id);
        ...
        $changes = array();
        if ($old_data) {
            if ($old_data->guide_work_name != $data['guide_work_name']) {
                $changes['guide_work_name'] = array(
                    'old' => $old_data->guide_work_name,
                    'new' => $data['guide_work_name']
                );
            }
            if ($old_data->guide_work_detail != $data['guide_work_detail']) {
                $changes['guide_work_detail'] = array(
                    'old' => 'มีการเปลี่ยนแปลง',
                    'new' => 'รายละเอียดใหม่'
                );
            }
            if ($old_data->guide_work_date != $data['guide_work_date']) {
                $changes['guide_work_date'] = array(
                    'old' => $old_data->guide_work_date,
                    'new' => $data['guide_work_date']
                );
            }
            if ($old_data->guide_work_link != $data['guide_work_link']) {
                $changes['guide_work_link'] = array(
                    'old' => $old_data->guide_work_link,
                    'new' => $data['guide_work_link']
                );
            }
        }
        ...
        if (!empty($imgs_data)) {
            ...
            $files_info['image_names'] = array_column($imgs_data, 'guide_work_img_img');
        }
        if (!empty($pdf_data)) {
            $files_info['pdfs_added'] = count($pdf_data);
            $files_info['pdf_names'] = array_column($pdf_data, 'guide_work_pdf_pdf');
        }
        if (!empty($doc_data)) {
            $files_info['docs_added'] = count($doc_data);
            $files_info['doc_names'] = array_column($doc_data, 'guide_work_file_doc');
        }
