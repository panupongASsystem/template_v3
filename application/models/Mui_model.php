...
class Mui_model extends CI_Model
{
    ...
    public function read_pdf($mui_id)
    {
        ...
        usort($results, function ($a, $b) {
            return strnatcmp($a->mui_pdf_pdf, $b->mui_pdf_pdf);
        });
    ...
    public function read_doc($mui_id)
    {
        ...
        usort($results, function ($a, $b) {
            return strnatcmp($a->mui_file_doc, $b->mui_file_doc);
        });
    ...
    public function read_img($mui_id)
    {
        ...
        $results = $query->result();
        usort($results, function ($a, $b) {
            return strnatcmp($a->mui_img_img, $b->mui_img_img);
        });
        return $results;
    }
    ...
    public function del_pdf($pdf_id)
    {
        ...
        $this->db->select('mui_pdf_pdf, mui_pdf_ref_id');
        $this->db->where('mui_pdf_id', $pdf_id);
        $query = $this->db->get('tbl_mui_pdf');
        $file_data = $query->row();
		
		// บันทึก log การลบไฟล์ PDF =============================================
        $old_data = $this->read($file_data->mui_pdf_ref_id);
        // =======================================================================
        ...
        $this->log_model->add_log(
            'ลบ',
            'มาตรการภายในหน่วย',
            'ไฟล์ PDF: ' . $file_data->mui_pdf_pdf . ' จาก: ' . ($old_data ? $old_data->mui_name : 'ไม่ระบุ'),
            $file_data->mui_pdf_ref_id,
            array('file_type' => 'PDF', 'file_name' => $file_data->mui_pdf_pdf)
        );
        // =======================================================================
    ...
    public function del_doc($doc_id)
    {
        ...
        $this->db->select('mui_file_doc, mui_file_ref_id');
        $this->db->where('mui_file_id', $doc_id);
        $query = $this->db->get('tbl_mui_file');
        $file_data = $query->row();
		
		// บันทึก log การลบไฟล์ DOC ========================================
        $old_data = $this->read($file_data->mui_file_ref_id);
        // ================================================================
        ...
        $this->log_model->add_log(
            'ลบ',
            'มาตรการภายในหน่วย',
            'ไฟล์ DOC: ' . $file_data->mui_file_doc . ' จาก: ' . ($old_data ? $old_data->mui_name : 'ไม่ระบุ'),
            $file_data->mui_file_ref_id,
            array('file_type' => 'DOC', 'file_name' => $file_data->mui_file_doc)
        );
        // ================================================================
        $this->session->set_flashdata('del_success', TRUE);
    }
    ...
    public function del_img($file_id)
    {
        ...
        $this->db->select('mui_img_img, mui_img_ref_id');
        $this->db->where('mui_img_id', $file_id);
        $query = $this->db->get('tbl_mui_img');
        $file_data = $query->row();
		
		// บันทึก log การลบรูปภาพ =================================================
        $old_data = $this->read($file_data->mui_img_ref_id);
        // =======================================================================
        ...
        $this->log_model->add_log(
            'ลบ',
            'มาตรการภายในหน่วย',
            'รูปภาพ: ' . $file_data->mui_img_img . ' จาก: ' . ($old_data ? $old_data->mui_name : 'ไม่ระบุ'),
            $file_data->mui_img_ref_id,
            array('file_type' => 'IMAGE', 'file_name' => $file_data->mui_img_img)
        );
        // =======================================================================
        $this->session->set_flashdata('del_success', TRUE);
    }
    ...
    public function edit($mui_id)
    {
        ...
        $changes = array();
        if ($old_data) {
            if ($old_data->mui_name != $data['mui_name']) {
                $changes['mui_name'] = array(
                    'old' => $old_data->mui_name,
                    'new' => $data['mui_name']
                );
            }
            if ($old_data->mui_detail != $data['mui_detail']) {
                $changes['mui_detail'] = array(
                    'old' => 'มีการเปลี่ยนแปลง',
                    'new' => 'รายละเอียดใหม่'
                );
            }
            if ($old_data->mui_date != $data['mui_date']) {
                $changes['mui_date'] = array(
                    'old' => $old_data->mui_date,
                    'new' => $data['mui_date']
                );
            }
            if ($old_data->mui_link != $data['mui_link']) {
                $changes['mui_link'] = array(
                    'old' => $old_data->mui_link,
                    'new' => $data['mui_link']
                );
            }
        }
        ...
        if (!empty($imgs_data)) {
            ...
            $files_info['image_names'] = array_column($imgs_data, 'mui_img_img');
        }
        if (!empty($pdf_data)) {
            $files_info['pdfs_added'] = count($pdf_data);
            $files_info['pdf_names'] = array_column($pdf_data, 'mui_pdf_pdf');
        }
        if (!empty($doc_data)) {
            $files_info['docs_added'] = count($doc_data);
            $files_info['doc_names'] = array_column($doc_data, 'mui_file_doc');
        }
