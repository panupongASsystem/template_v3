...
class Plan_pop_model extends CI_Model
{
    ...
    public function read_pdf($plan_pop_id)
    {
        ...
        usort($results, function ($a, $b) {
            return strnatcmp($a->plan_pop_pdf_pdf, $b->plan_pop_pdf_pdf);
        });
    ...
    public function read_doc($plan_pop_id)
    {
        ...
        usort($results, function ($a, $b) {
            return strnatcmp($a->plan_pop_file_doc, $b->plan_pop_file_doc);
        });
    ...
    public function read_img($plan_pop_id)
    {
        ...
        $results = $query->result();
        usort($results, function ($a, $b) {
            return strnatcmp($a->plan_pop_img_img, $b->plan_pop_img_img);
        });
        return $results;
    }
    ...
    public function del_pdf($pdf_id)
    {
        ...
        $this->db->select('plan_pop_pdf_pdf, plan_pop_pdf_ref_id');
        $this->db->where('plan_pop_pdf_id', $pdf_id);
        $query = $this->db->get('tbl_plan_pop_pdf');
        $file_data = $query->row();
		
		// บันทึก log การลบไฟล์ PDF =============================================
        $old_data = $this->read($file_data->plan_pop_pdf_ref_id);
        // =======================================================================
        ...
        $this->log_model->add_log(
            'ลบ',
            'แผนปฏิบัติการจัดซื้อจัดจ้าง',
            'ไฟล์ PDF: ' . $file_data->plan_pop_pdf_pdf . ' จาก: ' . ($old_data ? $old_data->plan_pop_name : 'ไม่ระบุ'),
            $file_data->plan_pop_pdf_ref_id,
            array('file_type' => 'PDF', 'file_name' => $file_data->plan_pop_pdf_pdf)
        );
        // =======================================================================
    ...
    public function del_doc($doc_id)
    {
        ...
        $this->db->select('plan_pop_file_doc, plan_pop_file_ref_id');
        $this->db->where('plan_pop_file_id', $doc_id);
        $query = $this->db->get('tbl_plan_pop_file');
        $file_data = $query->row();
		
		 // บันทึก log การลบไฟล์ DOC ========================================
        $old_data = $this->read($file_data->plan_pop_file_ref_id);
        // ================================================================
        ...
        $this->log_model->add_log(
            'ลบ',
            'แผนปฏิบัติการจัดซื้อจัดจ้าง',
            'ไฟล์ DOC: ' . $file_data->plan_pop_file_doc . ' จาก: ' . ($old_data ? $old_data->plan_pop_name : 'ไม่ระบุ'),
            $file_data->plan_pop_file_ref_id,
            array('file_type' => 'DOC', 'file_name' => $file_data->plan_pop_file_doc)
        );
        // ================================================================
        $this->session->set_flashdata('del_success', TRUE);
    }
    ...
    public function del_img($file_id)
    {
        ...
        $this->db->select('plan_pop_img_img, plan_pop_img_ref_id');
        $this->db->where('plan_pop_img_id', $file_id);
        $query = $this->db->get('tbl_plan_pop_img');
        $file_data = $query->row();
		
		// บันทึก log การลบรูปภาพ =================================================
        $old_data = $this->read($file_data->plan_pop_img_ref_id);
        // =======================================================================
        ...
        $this->log_model->add_log(
            'ลบ',
            'แผนปฏิบัติการจัดซื้อจัดจ้าง',
            'รูปภาพ: ' . $file_data->plan_pop_img_img . ' จาก: ' . ($old_data ? $old_data->plan_pop_name : 'ไม่ระบุ'),
            $file_data->plan_pop_img_ref_id,
            array('file_type' => 'IMAGE', 'file_name' => $file_data->plan_pop_img_img)
        );
        // =======================================================================
        $this->session->set_flashdata('del_success', TRUE);
    }
    ...
    public function edit($plan_pop_id)
    {
        ...
        $old_data = $this->read($plan_pop_id);
        ...
        $changes = array();
        if ($old_data) {
            if ($old_data->plan_pop_name != $data['plan_pop_name']) {
                $changes['plan_pop_name'] = array(
                    'old' => $old_data->plan_pop_name,
                    'new' => $data['plan_pop_name']
                );
            }
            if ($old_data->plan_pop_detail != $data['plan_pop_detail']) {
                $changes['plan_pop_detail'] = array(
                    'old' => 'มีการเปลี่ยนแปลง',
                    'new' => 'รายละเอียดใหม่'
                );
            }
            if ($old_data->plan_pop_date != $data['plan_pop_date']) {
                $changes['plan_pop_date'] = array(
                    'old' => $old_data->plan_pop_date,
                    'new' => $data['plan_pop_date']
                );
            }
            if ($old_data->plan_pop_link != $data['plan_pop_link']) {
                $changes['plan_pop_link'] = array(
                    'old' => $old_data->plan_pop_link,
                    'new' => $data['plan_pop_link']
                );
            }
        }
        ...
        if (!empty($imgs_data)) {
            ...
            $files_info['image_names'] = array_column($imgs_data, 'plan_pop_img_img');
        }
        if (!empty($pdf_data)) {
            $files_info['pdfs_added'] = count($pdf_data);
            $files_info['pdf_names'] = array_column($pdf_data, 'plan_pop_pdf_pdf');
        }
        if (!empty($doc_data)) {
            $files_info['docs_added'] = count($doc_data);
            $files_info['doc_names'] = array_column($doc_data, 'plan_pop_file_doc');
        }
