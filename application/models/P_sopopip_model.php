...
class P_sopopip_model extends CI_Model
{
    ...
    public function read_pdf($p_sopopip_id)
    {
        ...
        usort($results, function ($a, $b) {
            return strnatcmp($a->p_sopopip_pdf_pdf, $b->p_sopopip_pdf_pdf);
        });
    ...
    public function read_doc($p_sopopip_id)
    {
        ...
        usort($results, function ($a, $b) {
            return strnatcmp($a->p_sopopip_file_doc, $b->p_sopopip_file_doc);
        });
    ...
    public function read_img($p_sopopip_id)
    {
        ...
        $results = $query->result();
        usort($results, function ($a, $b) {
            return strnatcmp($a->p_sopopip_img_img, $b->p_sopopip_img_img);
        });
        return $results;
    }
    ...
    public function del_pdf($pdf_id)
    {
        ...
        $this->db->select('p_sopopip_pdf_pdf, p_sopopip_pdf_ref_id');
        $this->db->where('p_sopopip_pdf_id', $pdf_id);
        $query = $this->db->get('tbl_p_sopopip_pdf');
        $file_data = $query->row();
		
		// บันทึก log การลบไฟล์ PDF =============================================
        $old_data = $this->read($file_data->p_sopopip_pdf_ref_id);
        // =======================================================================
        ...
        $this->log_model->add_log(
            'ลบ',
            'รายงานความก้าวหน้าการจัดซื้อจัดจ้างหรือการจัดหาพัสดุ',
            'ไฟล์ PDF: ' . $file_data->p_sopopip_pdf_pdf . ' จาก: ' . ($old_data ? $old_data->p_sopopip_name : 'ไม่ระบุ'),
            $file_data->p_sopopip_pdf_ref_id,
            array('file_type' => 'PDF', 'file_name' => $file_data->p_sopopip_pdf_pdf)
        );
        // =======================================================================
    ...
    public function del_doc($doc_id)
    {
        ...
        $this->db->select('p_sopopip_file_doc, p_sopopip_file_ref_id');
        $this->db->where('p_sopopip_file_id', $doc_id);
        $query = $this->db->get('tbl_p_sopopip_file');
        $file_data = $query->row();
		
		// บันทึก log การลบไฟล์ DOC ========================================
        $old_data = $this->read($file_data->p_sopopip_file_ref_id);
        // ================================================================
        ...
        $this->log_model->add_log(
            'ลบ',
            'รายงานความก้าวหน้าการจัดซื้อจัดจ้างหรือการจัดหาพัสดุ',
            'ไฟล์ DOC: ' . $file_data->p_sopopip_file_doc . ' จาก: ' . ($old_data ? $old_data->p_sopopip_name : 'ไม่ระบุ'),
            $file_data->p_sopopip_file_ref_id,
            array('file_type' => 'DOC', 'file_name' => $file_data->p_sopopip_file_doc)
        );
        // ================================================================
        $this->session->set_flashdata('del_success', TRUE);
    }
    ...
    public function del_img($file_id)
    {
        ...
        $this->db->select('p_sopopip_img_img, p_sopopip_img_ref_id');
        $this->db->where('p_sopopip_img_id', $file_id);
        $query = $this->db->get('tbl_p_sopopip_img');
        $file_data = $query->row();
		
		// บันทึก log การลบรูปภาพ =================================================
        $old_data = $this->read($file_data->p_sopopip_img_ref_id);
        // =======================================================================
        ...
        $this->log_model->add_log(
            'ลบ',
            'รายงานความก้าวหน้าการจัดซื้อจัดจ้างหรือการจัดหาพัสดุ',
            'รูปภาพ: ' . $file_data->p_sopopip_img_img . ' จาก: ' . ($old_data ? $old_data->p_sopopip_name : 'ไม่ระบุ'),
            $file_data->p_sopopip_img_ref_id,
            array('file_type' => 'IMAGE', 'file_name' => $file_data->p_sopopip_img_img)
        );
        // =======================================================================
        $this->session->set_flashdata('del_success', TRUE);
    }
    ...
    public function edit($p_sopopip_id)
    {
        ...
        $old_data = $this->read($p_sopopip_id);
        ...
        $changes = array();
        if ($old_data) {
            if ($old_data->p_sopopip_name != $data['p_sopopip_name']) {
                $changes['p_sopopip_name'] = array(
                    'old' => $old_data->p_sopopip_name,
                    'new' => $data['p_sopopip_name']
                );
            }
            if ($old_data->p_sopopip_detail != $data['p_sopopip_detail']) {
                $changes['p_sopopip_detail'] = array(
                    'old' => 'มีการเปลี่ยนแปลง',
                    'new' => 'รายละเอียดใหม่'
                );
            }
            if ($old_data->p_sopopip_date != $data['p_sopopip_date']) {
                $changes['p_sopopip_date'] = array(
                    'old' => $old_data->p_sopopip_date,
                    'new' => $data['p_sopopip_date']
                );
            }
            if ($old_data->p_sopopip_link != $data['p_sopopip_link']) {
                $changes['p_sopopip_link'] = array(
                    'old' => $old_data->p_sopopip_link,
                    'new' => $data['p_sopopip_link']
                );
            }
        }
        ...
        if (!empty($imgs_data)) {
            ...
            $files_info['image_names'] = array_column($imgs_data, 'p_sopopip_img_img');
        }
        if (!empty($pdf_data)) {
            $files_info['pdfs_added'] = count($pdf_data);
            $files_info['pdf_names'] = array_column($pdf_data, 'p_sopopip_pdf_pdf');
        }
        if (!empty($doc_data)) {
            $files_info['docs_added'] = count($doc_data);
            $files_info['doc_names'] = array_column($doc_data, 'p_sopopip_file_doc');
        }
