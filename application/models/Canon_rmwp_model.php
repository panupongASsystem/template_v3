...
class Canon_rmwp_model extends CI_Model
{
    ...
    public function add()
    {
        ...
        $this->log_model->add_log(
            'เพิ่ม',
            'บัญญัติการจัดการสิ่งปฏิกูลและมูลฝอย',
            $canon_rmwp_data['canon_rmwp_name'],
            $canon_rmwp_id,
            array(
    ...
    public function read_pdf($canon_rmwp_id)
    {
        ...
        usort($results, function ($a, $b) {
            return strnatcmp($a->canon_rmwp_pdf_pdf, $b->canon_rmwp_pdf_pdf);
        });
    ...
    public function read_doc($canon_rmwp_id)
    {
        ...
        usort($results, function ($a, $b) {
            return strnatcmp($a->canon_rmwp_file_doc, $b->canon_rmwp_file_doc);
        });
    ...
    public function read_img($canon_rmwp_id)
    {
        ...
        $results = $query->result();
        usort($results, function ($a, $b) {
            return strnatcmp($a->canon_rmwp_img_img, $b->canon_rmwp_img_img);
        });
        return $results;
    }
    ...
    public function del_pdf($pdf_id)
    {
        ...
        $this->db->select('canon_rmwp_pdf_pdf, canon_rmwp_pdf_ref_id');
        $this->db->where('canon_rmwp_pdf_id', $pdf_id);
        $query = $this->db->get('tbl_canon_rmwp_pdf');
        $file_data = $query->row();
		
		 // บันทึก log การลบไฟล์ PDF =============================================
        $old_data = $this->read($file_data->canon_rmwp_pdf_ref_id);
        // =======================================================================
        ...
        $this->log_model->add_log(
            'ลบ',
            'บัญญัติการจัดการสิ่งปฏิกูลและมูลฝอย',
            'ไฟล์ PDF: ' . $file_data->canon_rmwp_pdf_pdf . ' จาก: ' . ($old_data ? $old_data->canon_rmwp_name : 'ไม่ระบุ'),
            $file_data->canon_rmwp_pdf_ref_id,
            array('file_type' => 'PDF', 'file_name' => $file_data->canon_rmwp_pdf_pdf)
        );
        // =======================================================================
    ...
    public function del_doc($doc_id)
    {
        ...
        $this->db->select('canon_rmwp_file_doc, canon_rmwp_file_ref_id');
        $this->db->where('canon_rmwp_file_id', $doc_id);
        $query = $this->db->get('tbl_canon_rmwp_file');
        $file_data = $query->row();
		
		// บันทึก log การลบไฟล์ DOC ========================================
        $old_data = $this->read($file_data->canon_rmwp_file_ref_id);
        // ================================================================
        ...
        $this->log_model->add_log(
            'ลบ',
            'บัญญัติการจัดการสิ่งปฏิกูลและมูลฝอย',
            'ไฟล์ DOC: ' . $file_data->canon_rmwp_file_doc . ' จาก: ' . ($old_data ? $old_data->canon_rmwp_name : 'ไม่ระบุ'),
            $file_data->canon_rmwp_file_ref_id,
            array('file_type' => 'DOC', 'file_name' => $file_data->canon_rmwp_file_doc)
        );
        // ================================================================
        $this->session->set_flashdata('del_success', TRUE);
    }
    ...
    public function del_img($file_id)
    {
        ...
        $this->db->select('canon_rmwp_img_img, canon_rmwp_img_ref_id');
        $this->db->where('canon_rmwp_img_id', $file_id);
        $query = $this->db->get('tbl_canon_rmwp_img');
        $file_data = $query->row();
		
		// บันทึก log การลบรูปภาพ =================================================
        $old_data = $this->read($file_data->canon_rmwp_img_ref_id);
        // =======================================================================
        ...
        $this->log_model->add_log(
            'ลบ',
            'บัญญัติการจัดการสิ่งปฏิกูลและมูลฝอย',
            'รูปภาพ: ' . $file_data->canon_rmwp_img_img . ' จาก: ' . ($old_data ? $old_data->canon_rmwp_name : 'ไม่ระบุ'),
            $file_data->canon_rmwp_img_ref_id,
            array('file_type' => 'IMAGE', 'file_name' => $file_data->canon_rmwp_img_img)
        );
        // =======================================================================
        $this->session->set_flashdata('del_success', TRUE);
    }
    ...
    public function edit($canon_rmwp_id)
    {
        ...
        $old_data = $this->read($canon_rmwp_idd);
        ...
        $changes = array();
        if ($old_data) {
            if ($old_data->canon_rmwp_name != $data['canon_rmwp_name']) {
                $changes['canon_rmwp_name'] = array(
                    'old' => $old_data->canon_rmwp_name,
                    'new' => $data['canon_rmwp_name']
                );
            }
            if ($old_data->canon_rmwp_detail != $data['canon_rmwp_detail']) {
                $changes['canon_rmwp_detail'] = array(
                    'old' => 'มีการเปลี่ยนแปลง',
                    'new' => 'รายละเอียดใหม่'
                );
            }
            if ($old_data->canon_rmwp_date != $data['canon_rmwp_date']) {
                $changes['canon_rmwp_date'] = array(
                    'old' => $old_data->canon_rmwp_date,
                    'new' => $data['canon_rmwp_date']
                );
            }
            if ($old_data->canon_rmwp_link != $data['canon_rmwp_link']) {
                $changes['canon_rmwp_link'] = array(
                    'old' => $old_data->canon_rmwp_link,
                    'new' => $data['canon_rmwp_link']
                );
            }
        }
        ...
        if (!empty($imgs_data)) {
            ...
            $files_info['image_names'] = array_column($imgs_data, 'canon_rmwp_img_img');
        }
        if (!empty($pdf_data)) {
            $files_info['pdfs_added'] = count($pdf_data);
            $files_info['pdf_names'] = array_column($pdf_data, 'canon_rmwp_pdf_pdf');
        }
        if (!empty($doc_data)) {
            $files_info['docs_added'] = count($doc_data);
            $files_info['doc_names'] = array_column($doc_data, 'canon_rmwp_file_doc');
        }
